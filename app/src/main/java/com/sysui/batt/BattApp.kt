package com.sysui.batt

import android.app.Application
import android.content.Context
import com.sysui.batt.utils.helper.LocaleHelper
import com.google.android.material.color.DynamicColors
import java.lang.ref.WeakReference

class BattApp : Application() {

    companion object {
        private var instance: BattApp? = null
        private var contextReference: WeakReference<Context>? = null

        val appContext: Context
            get() {
                if (contextReference == null || contextReference?.get() == null) {
                    val app = instance?.applicationContext
                        ?: throw IllegalStateException("BattApp application is not created yet")
                    contextReference = WeakReference(app)
                }
                return contextReference!!.get()
                    ?: throw IllegalStateException("BattApp application context was garbage collected")
            }

        val appContextLocale: Context
            get() {
                return LocaleHelper.setLocale(appContext)
            }

        private fun getInstance(): BattApp {
            return instance
                ?: throw IllegalStateException("BattApp application is not created yet")
        }
    }

    override fun onCreate() {
        super.onCreate()
        instance = this
        contextReference = WeakReference(applicationContext)
        DynamicColors.applyToActivitiesIfAvailable(this)
    }
}