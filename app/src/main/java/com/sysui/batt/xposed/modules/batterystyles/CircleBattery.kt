package com.sysui.batt.xposed.modules.batterystyles
...
import com.sysui.batt.data.common.Preferences
...
import com.sysui.batt.xposed.modules.extras.utils.AlphaRefreshedPaint
...
open class CircleBattery(private val mContext: Context, frameColor: Int) : BatteryDrawable() {
    ...
    private var mLevelAnimator: ValueAnimator? = null
    private val expressiveInterpolator = PathInterpolator(0.05f, 0.7f, 0.1f, 1f)
    ...
    fun animateLevelTo(target: Int) {
        mLevelAnimator?.cancel()
        val from = batteryLevel
        mLevelAnimator = ValueAnimator.ofInt(from, target.coerceIn(0, 100)).apply {
            duration = 400
            interpolator = expressiveInterpolator
            addUpdateListener { batteryLevel = it.animatedValue as Int; invalidateSelf() }
            start()
        }
    }

    fun setMeterStyle(batteryStyle: Int) {
        val dotted = batteryStyle == BATTERY_STYLE_DOTTED_CIRCLE
        // Dotted must use BUTT caps: ROUND caps extend each dash by
        // strokeWidth/2 on both ends and close the 2px gap, making dotted
        // render identical to continuous. Continuous keeps expressive ROUND caps.
        mFramePaint.strokeCap = if (dotted) Paint.Cap.BUTT else Paint.Cap.ROUND
        mBatteryPaint.strokeCap = if (dotted) Paint.Cap.BUTT else Paint.Cap.ROUND
        mFramePaint.setPathEffect(if (dotted) DASH_PATH_EFFECT else null)
        mBatteryPaint.setPathEffect(if (dotted) DASH_PATH_EFFECT else null)
        invalidateSelf()
    }
    ...
    private fun updateSize() {
        ...
        val strokeWidth = mDiameter / 5.5f
    ...
    init {
        ...
        mBatteryPaint.strokeCap = Paint.Cap.ROUND
        mBatteryPaint.strokeJoin = Paint.Join.ROUND
        mBoltAlphaAnimator = ValueAnimator.ofInt(255, 255, 255, 45)
        mBoltAlphaAnimator.setDuration(2000)
        mBoltAlphaAnimator.interpolator = expressiveInterpolator
