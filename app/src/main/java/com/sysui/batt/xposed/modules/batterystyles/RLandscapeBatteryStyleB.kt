...
open class RLandscapeBatteryStyleB(private val context: Context, frameColor: Int) :
    BatteryDrawable() {
    ...
    private fun loadPaths() {
        val pathString =
            getResources(context).getString(R.string.config_landscapeBatteryPerimeterRStyleB)
        perimeterPath.set(PathParser.createPathFromPathData(pathString))
        perimeterPath.computeBounds(RectF(), true)

        val errorPathString =
            getResources(context).getString(R.string.config_landscapeBatteryErrorRStyleB)
        errorPerimeterPath.set(PathParser.createPathFromPathData(errorPathString))
        errorPerimeterPath.computeBounds(RectF(), true)

        val fillMaskString =
            getResources(context).getString(R.string.config_landscapeBatteryFillMaskRStyleB)
        fillMask.set(PathParser.createPathFromPathData(fillMaskString))
        // Set the fill rect so we can calculate the fill properly
        fillMask.computeBounds(fillRect, true)

        val boltPathString =
            getResources(context).getString(R.string.config_landscapeBatteryBoltRStyleB)
        boltPath.set(PathParser.createPathFromPathData(boltPathString))

        val plusPathString =
            getResources(context).getString(R.string.config_landscapeBatteryPlusRStyleB)
