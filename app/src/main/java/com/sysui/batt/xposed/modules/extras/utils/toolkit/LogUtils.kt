package com.sysui.batt.xposed.modules.extras.utils.toolkit
...
fun <T : Any> log(clazz: T, message: Any?) {
    try {
        Log.i(
            TAG,
            "Batt - ${clazz.javaClass.simpleName.replace("\$Companion", "")}: $message"
        )
    } catch (_: Throwable) {
    }
}
...
fun <T : Any> log(clazz: T, throwable: Throwable?) {
    try {
        Log.e(
            TAG,
            "Batt - ${clazz.javaClass.simpleName.replace("\$Companion", "")}: $throwable",
            throwable
        )
    } catch (_: Throwable) {
    }
