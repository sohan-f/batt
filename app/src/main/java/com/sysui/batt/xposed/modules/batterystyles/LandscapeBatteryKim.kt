...
open class LandscapeBatteryKim(private val context: Context, frameColor: Int) :
    BatteryDrawable() {
    ...
    override fun draw(c: Canvas) {
        ...
        // Define fillRect and fillFraction based on battery level
        levelRect.set(fillRect)
        val fillFraction = batteryLevel / 100f
        val fillTop =
            if (batteryLevel >= 95) fillRect.right else fillRect.right - (fillRect.width() * (1 - fillFraction))
        levelRect.right = floor(fillTop.toDouble()).toFloat()

        // Add scaledFill to levelPath
        levelPath.addPath(scaledFill)

        // Add scaledPerimeter to unifiedPath
        ...
        // Draw the fill mask on top
        fillPaint.alpha = 85
        c.drawPath(levelPath, fillPaint)

        // Draw the fill mask again, clipped from the right side based on battery percentage, with 1.0 opacity
        fillPaint.alpha = 255
        c.save()
        c.clipRect(
            bounds.left.toFloat(),
    ...
    private fun batteryColorForLevel(level: Int): Int {
        return when {
            charging -> 0xFF63D680.toInt()
            powerSaveEnabled -> 0xFFFCD84E.toInt()
            level > 20 -> fillColor
            level >= 0 -> 0xFFFF3030.toInt()
    ...
    private fun loadPaths() {
        val pathString =
            getResources(context).getString(R.string.config_landscapeBatteryPerimeterTest)
        perimeterPath.set(PathParser.createPathFromPathData(pathString))
        perimeterPath.computeBounds(RectF(), true)

        val errorPathString =
            getResources(context).getString(R.string.config_landscapeBatteryErrorTest)
        errorPerimeterPath.set(PathParser.createPathFromPathData(errorPathString))
        errorPerimeterPath.computeBounds(RectF(), true)

        val fillMaskString =
            getResources(context).getString(R.string.config_landscapeBatteryFillMaskTest)
        fillMask.set(PathParser.createPathFromPathData(fillMaskString))
        // Set the fill rect so we can calculate the fill properly
        fillMask.computeBounds(fillRect, true)

        val boltPathString =
            getResources(context).getString(R.string.config_landscapeBatteryBoltTest)
        boltPath.set(PathParser.createPathFromPathData(boltPathString))

        val plusPathString =
            getResources(context).getString(R.string.config_landscapeBatteryPlusTest)
    ...
    companion object {
        private val TAG = LandscapeBatteryKim::class.java.simpleName
        private const val WIDTH = 17f
