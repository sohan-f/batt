...
open class PortraitBatteryOrigami(private val context: Context, frameColor: Int) :
    ...
    private fun loadPaths() {
        ...
        val errorPathString =
            getResources(context).getString(R.string.config_portraitBatteryErrorOrigami)
        errorPerimeterPath.set(PathParser.createPathFromPathData(errorPathString))
        errorPerimeterPath.computeBounds(RectF(), true)

        val fillMaskString =
            getResources(context).getString(R.string.config_portraitBatteryFillMaskOrigami)
        fillMask.set(PathParser.createPathFromPathData(fillMaskString))
        // Set the fill rect so we can calculate the fill properly
        fillMask.computeBounds(fillRect, true)

        val boltPathString =
            getResources(context).getString(R.string.config_portraitBatteryBoltOrigami)
        boltPath.set(PathParser.createPathFromPathData(boltPathString))

        val plusPathString =
            getResources(context).getString(R.string.config_portraitBatteryPlusOrigami)
